...
combined_stations = np.array([("Addison-O'Hare", 2455, 917),
...
weekday_stations = np.array([("Addison-O'Hare", 2949, 519),
    ...
    ("Western-Forest Park", 1639, 270),
    ("Western/Milwaukee", 5370, 810)])


weekend_stations = np.array([("Addison-O'Hare", 1221, 363),
...
### SIM 1
### Sunny weather,  weekday commute, no event, trains from 50 - 250 incremented by 25.

numTrains = list(range(50, 251, 25))
sim1_CI = []
sim1_vars = []

for trains in numTrains:
    sim = TrainSimulation(weekday_stations, trains, 'sunny', 'False')
    result = sim.RunSimulation(100)
    var = sim.var(0.95)
    sim1_CI.append(result)
    sim1_vars.append(var)

sim1_mean = np.array(sim1_CI)[:,1]


### SIM 2
### Sunny weather,  weekend, no event, trains from 50 - 250 incremented by 25.

sim2_CI = []
sim2_vars = []

for trains in numTrains:
    sim = TrainSimulation(weekend_stations, trains, 'sunny', 'False')
    result = sim.RunSimulation(100)
    var = sim.var(0.95)
    sim2_CI.append(result)
    sim2_vars.append(var)

sim2_mean = np.array(sim2_CI)[:,1]

### SIM 3
### Cold weather,  weekday commute, no event, trains from 50 - 250 incremented by 25.
...
for trains in numTrains:
    sim = TrainSimulation(weekday_stations, trains, 'inclement', 'False')
    result = sim.RunSimulation(100)
    var = sim.var(0.95)
    sim3_CI.append(result)
    sim3_vars.append(var)

sim3_mean = np.array(sim3_CI)[:,1]

### SIM 4
### Cold weather,  weekend, no event, trains from 50 - 250 incremented by 25.

sim4_CI = []
sim4_vars = []

for trains in numTrains:
    sim = TrainSimulation(weekend_stations, trains, 'inclement', 'False')
    result = sim.RunSimulation(100)
    var = sim.var(0.95)
    sim4_CI.append(result)
    sim4_vars.append(var)

sim4_mean = np.array(sim4_CI)[:,1]

plt.plot(numTrains, sim1_mean, label = "Sunny Commute Mean")
plt.plot(numTrains, sim2_mean, label = "Sunny Weekend Mean")
plt.plot(numTrains, sim3_mean, label = "Inclement Commute Mean")
plt.plot(numTrains, sim4_mean, label = "Inclement Weekend Mean")
plt.xlabel("Number of trains in day")
plt.ylabel("Number of riders unable to catch a train")
plt.title("Mean Riders Compared over Commute and Weather")
plt.legend()
plt.show()
...
### Sunny, weekday, big event, trains from 50-350 incremented by 25

numTrains2 = list(range(50, 351, 25))
sim5_CI = []
sim5_vars = []

for trains in numTrains2:
    sim = TrainSimulation(weekday_stations, trains, 'sunny', 'True')
    result = sim.RunSimulation(100)
    var = sim.var(0.95)
    sim5_CI.append(result)
    sim5_vars.append(var)

plt.plot(numTrains2, sim5_vars, label = "High-End Outlier")
plt.plot(numTrains, sim1_vars, label = "Sunny Commute")
plt.legend()
plt.title("VAR for Regular Commute vs High-End Outlier")
plt.xlabel("Number of trains in day")
plt.ylabel("Number of riders unable to catch a train")
plt.show()
...
## Inclement weekend VAR

numTrains3 = list(range(5, 76, 5))
sim6_CI = []
sim6_vars = []

for trains in numTrains3:
    sim = TrainSimulation(weekend_stations, trains, 'inclement', 'False')
    result = sim.RunSimulation(100)
    var = sim.var(0.05)
    sim6_CI.append(result)
    sim6_vars.append(var)


plt.plot(numTrains3, sim6_vars, label = "Low-End Outlier")
plt.plot(numTrains, sim1_vars, label = "Sunny Commute")
plt.legend()
plt.title("VAR (5%) for Regular Commute vs Low-End Outlier")
plt.xlabel("Number of trains in day")
plt.ylabel("Number of riders unable to catch a train")
plt.show()